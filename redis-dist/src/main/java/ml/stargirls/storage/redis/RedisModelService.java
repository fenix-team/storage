...
public class RedisModelService<T extends Model, Reader extends ModelReader<Reader, JsonObject>>
	...
	protected RedisModelService(
			@NotNull Executor executor, @NotNull Gson gson, @NotNull Function<JsonObject, Reader> readerFactory,
			@NotNull ModelCodec.Writer<T, JsonObject> writer, @NotNull ModelCodec.Reader<T, JsonObject, Reader> reader,
			@NotNull JedisPool jedisPool, @NotNull String tableName,
			int expireAfterSave, int expireAfterAccess
	) {
		super(executor);
		this.gson = gson;
